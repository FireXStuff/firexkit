...
class FxAsyncResult(AsyncResult, Generic[R]):
    ...
    def update_parent_task_blocked_states(
        ...
        if parent_id:
            with cls.app.events.default_dispatcher(
    ...
    def fx_wait(
        ...
        if not self.fx_is_ready():
            ...
            # is already complete
            parent_id = None
        with self.update_parent_task_blocked_states(parent_id):
...
class ManyFxAsyncResults(Generic[K]):
    ...
    def get_as_completed(
        ...
        if first_ar := next(self._fx_ars_by_key.values(), None):
            with FxAsyncResult.update_parent_task_blocked_states(
                # assume all ars have same parent.
                first_ar.fx_parent_id(),
            ):
...
def _is_worker_alive(result: FxAsyncResult) -> bool:
    ...
    while tries <= retries:
        ...
        if state in [STARTED, RECEIVED]:
            ...
            if not hostname:
                logger.debug(f'Cannot get run info for {result.fx_logging_name()}; assuming task is alive.'
